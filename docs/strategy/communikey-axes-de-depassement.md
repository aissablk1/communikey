---
title: "communikey — Tous les axes de dépassement (analyse concurrentielle)"
date: 2026-07-01
...
# communikey — Tous les axes de dépassement
...
1. **« Surpasser ClaudeKit » est un faux combat.** ClaudeKit (theclaudekit.com) et les repos
   ...
   *« They inspect and report. They do not orchestrate other agents… There are no reviewer gates that
   block completion. »* communikey (un **bus de coordination**) n'est pas dans leur catégorie ; il ne les
...
recovery). **C'est là que communikey est structurellement supérieur.**

---

## 1. Clarification de catégorie (ne pas confondre)

| Catégorie | Ce que c'est | Exemples | communikey concurrent ? |
|---|---|---|---|
...
| **Bus de coordination** | *comment des sessions vivantes se parlent* | **communikey**, **hcom**, MCP Agent Mail, murmur, Agent Teams natif | **Oui** — c'est le terrain de communikey |

Corollaire : accumuler des skills pour « dépasser » les kits est le **piège** (§57) — un solo OSS ne
gagne pas une guerre de contenu contre une boîte + 1000+ repos. communikey gagne sur le **transport**.
...
|---|:---:|:---:|:---:|:---:|:---:|:---:|---|---|
| **communikey** | ◐ (Claude ✅ ; Codex/Gemini calibrés) | ✅ (LAN, TLS hybride PQC) | ✅ **asym. par destinataire** | ✅ **Ed25519 signé** | ✅ **ML-KEM-768** | ✅ **Shamir + BIP-39** | Apache-2.0 | alpha v0.2.0 |
...
**Lecture :** hcom domine la **largeur** et la **maturité** ; communikey domine la **confiance
cryptographique** (auth, PQC, recovery) et la **souveraineté**. Ce sont deux profils miroirs.

## 3. Tous les axes, scorés (communikey vs hcom, 1-5, honnête)

| # | Axe | communikey | hcom | Note |
|---|---|:---:|:---:|---|
| 1 | Modèle de coordination (live inter-sessions) | 5 | 5 | Les deux le font vraiment |
| 2 | **Largeur cross-provider** | 3 | **5** | hcom = 10 CLIs auto ; communikey = Claude solide, Codex/Gemini calibrés non-confirmés live |
| 3 | Cross-machine | 4 | 5 | hcom MQTT mûr ; communikey LAN + TLS hybride PQC (phase 3) |
| 4 | Chiffrement des messages | **5** | 4 | communikey asymétrique **par destinataire** ; hcom **PSK partagé** (un secret = tout le relay) |
| 5 | **Auth d'expéditeur (provenance)** | **5** | **2** | **Killer axis.** communikey signe Ed25519 ; hcom : *« sender identity is not authorization »* |
| 6 | Post-quantique | **5** | 1 | communikey ML-KEM-768 hybride ; hcom : non mentionné |
| 7 | Gestion de clés + recovery | **5** | 2 | communikey identités/vault/Shamir/BIP-39 ; hcom PSK unique, pas de recovery |
| 8 | Intégration turnkey (livraison) | 3 | **5** | hcom hooks auto sur 10 CLIs ; communikey inbox universel + injection clavier Unix (plus de friction) |
| 9 | Livraison state-aware / sûreté | 4 | 4 | communikey safety-first (jamais valider un y/N) ; hcom statut active/blocked + inject via hooks |
| 10 | Souveraineté / coût | **5** | 4 | Les deux libres ; communikey relay auto-hébergé + PQC, cadrage souverain |
| 11 | **Maturité / adoption** | 2 | **4** | hcom établi (~363★, 10 CLIs) ; communikey alpha d'un jour, zéro adoption |
| 12 | Brand / docs / polish | 3 | 4 | hcom rodé ; communikey docs bonnes mais neuf |

**Profil communikey :** exceptionnel sur **4-5-6-7-10** (confiance/PQC/souveraineté), en **retard sur
2-8-11-12** (largeur, intégration turnkey, maturité, notoriété).

## 4. SWOT (communikey vs hcom)
...
**communikey = le bus d'agents cryptographiquement authentifié, post-quantique et souverain** — pour qui
**ne peut pas** faire confiance à un PSK partagé (environnements régulés, multi-tenant, sensibles,
adversariaux). *« Un mot de passe partagé ne dit pas QUI parle. communikey, si — signé, chiffré par
...
|---|---|---|
| **Quick (0-2 sem)** | Recadrer le pitch communikey autour de **provenance signée + PQC + souveraineté** (PAS « vs ClaudeKit »). Page de comparaison **honnête vs hcom** (forces ET retards). Rendre lisible l'axe « auth d'expéditeur ». | Moat |
...
| **Medium** | Couche kit **mince** : quelques commandes communikey-natives d'orchestration (dispatch, hand-off, coordination inter-sessions signée). Pas 5 verticales. | Kit mince |
...
communikey **peut** surpasser — mais **sur l'axe confiance/sécurité/souveraineté**, pas sur la largeur ni
...
2. **Mesurer communikey contre `hcom`**, le vrai concurrent — et **assumer les retards** (largeur,
...
**Le dépassement réel de communikey = devenir le bus d'agents que l'on peut auditer et auquel on peut faire
