...
« Hermes » mérite une clarification avant tout calibrage : c’est aussi le nom de ton pont MCP
de messagerie (Telegram/Discord/Slack/WhatsApp/Signal/Matrix) déjà installé — il faut vérifier
si cmux référence le même outil ou un CLI d’agent distinct avant d’écrire le moindre pattern
de détection à son sujet.
